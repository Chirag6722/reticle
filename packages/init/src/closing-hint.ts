...
export function restartHint(
  ...
  return (
    ...
    'ask a human to do it: `npx @reticlehq/server init --relaunch` restarts the client for you and ' +
    'resumes this work on the other side. Everywhere else it is a human step — reload the window ' +
    'in Cursor, or hit Start in `.vscode/mcp.json` in VS Code — so name the one that applies and ' +
    'ask for that alone.\n' +
