...
`init` wrote the file, but a brand-new Vite template has no testids and no state library, so there was nothing to register. Reticle tells you this instead of counting it as a win. See [Instrument your app](/integration-patterns) for what to put in it.
...
<AccordionGroup>
  ...
  <Accordion title="vite.config.ts — the plugin">
    ...
    ```
    One import, one entry. The plugin stamps source locations onto your elements — that's what turns
    a DOM node into `src/components/Login.tsx:81` — and injects `connect()` so you don't have to
    remember to.
  ...
  <Accordion title="CLAUDE.md and .claude/commands/reticle.md — agent rules">
