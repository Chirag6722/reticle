...
## Already installed? Re-run init

Nothing reaches back into a machine that installed Reticle on an earlier version. There is no migration step and no background updater. New wiring arrives when you next run `init`, and only then.
