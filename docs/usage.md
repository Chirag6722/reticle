...
**`inputModeReason` — never a silent fallback.** When real input **is** configured but a pointer act still ran synthetic, the result says _why_, so per-element inconsistency is diagnosable instead of mysterious:
