...
<CardGroup cols={2}>
  <Card title="Mock data" icon="ghost">
    The POST returns `200`. The row appears in the table. Reload the page and it's gone — nothing
    was ever persisted.
  </Card>
...
We built eleven scenarios — ten bugs plus a control where nothing is wrong, so that anything flagged on the control counts as a false positive. Each tool got the same app, the same moment, the same task.
