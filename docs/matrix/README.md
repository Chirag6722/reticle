...
Everything in here was submitted against **2.5.0**. The repo ships **2.13.1**. Nothing has been re-measured since, so [`MATRIX.md`](./MATRIX.md) carries a banner saying so, and `matrix-freshness.test.ts` fails the unit gate if that banner is missing or names the wrong versions.
