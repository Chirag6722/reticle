...
function actLink(actionId: string, attribution: string | undefined): LineageLink {
  // Rendered from the tier the EVENT carries, never from a constant of our own. The schema says
  // `attribution` is present iff `actionId` is, so on this path it always is; the guard against
  // its absence exists so a contract slip renders a shorter sentence rather than a wrong one.
  const tier = attribution === undefined ? '' : ` — ${attribution} tier`;
  return {
    kind: 'act',
    text: `← attributed to action ${actionId}${tier}: the SDK stamped this action's id on every event between its dispatch and settle. A time window, not dataflow.`,
    observed: false,
  };
}
