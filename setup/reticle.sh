...
command -v node >/dev/null 2>&1 || {
  echo "reticle setup needs Node (it is what your app's own dev server runs on): https://nodejs.org" >&2
  exit 1
}
...
node -e 'process.exit(Number(process.versions.node.split(".")[0]) >= 18 ? 0 : 1)' 2>/dev/null || {
  echo "reticle setup needs Node 18 or newer; this is $(node -v 2>/dev/null || echo 'an unknown version')." >&2
  echo "Node 16 and older have no global fetch, so setup would edit your build config and then die halfway through." >&2
  exit 1
}
